...
public class SqliteBookDAO implements BookDAO {
    ...
    public Integer create(Book book) {
        String sql = "INSERT INTO Book(name, authors, publisher, isbn, edition, " +
                "num_of_pages, gender, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        try(PreparedStatement stmt = ConnectionFactory.createPreparedStatement(sql)) {
            ...
            stmt.setString(7, book.getGender().toString());
    ...
    private Book resultSetToEntity(ResultSet rs) throws SQLException {
        return new Book(
                ...
                rs.getString("isbn"),
                BookGender.toEnum(rs.getString("gender")),
    ...
    public boolean update(Book book) {
        String sql = "UPDATE Book SET name = ?, author = ?, publisher = ?, isbn = ?, edition = ?, " +
                "num_of_pages = ?, gender = ?, status = ? WHERE id = ?";
        try(PreparedStatement stmt = ConnectionFactory.createPreparedStatement(sql)) {
            stmt.setString(1, book.getName());
            stmt.setString(2, book.getAuthors());
            stmt.setString(3, book.getPublisher());
