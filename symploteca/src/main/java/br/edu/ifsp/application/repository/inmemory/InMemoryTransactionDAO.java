...
public class InMemoryTransactionDAO implements TransactionDAO {
    ...
    @Override
    public Optional<Transaction> findByDataRange(LocalDate initialDate, LocalDate finalDate, String institutuionallId, Integer id) {
        return db.values().stream()
                .filter(transaction -> transaction.getIssueDate().isBefore(initialDate))
                .filter(transaction -> transaction.getIssueDate().isAfter(finalDate))
                .filter(transaction -> transaction.getUser().getInstitutionallId().equals(institutuionallId))
                .filter(transaction -> transaction.getBook().getId() == id)
                .findAny();
    }
