...
public class MainUIController {
    ...
    public void borrowOrReturn(ActionEvent actionEvent) {
        if (selectedBook != null) {
            if (selectedBook.getStatus() == BookStatus.AVAILABLE)
                if (user == null)
                    showAlert("Erro!", "O leitor deve ser informado para a realização do empréstimo", Alert.AlertType.ERROR);
                else
    ...
    private void borrowBook(Book selectedBook) {
        try {
            Transaction transaction = checkOutTransactionUseCase.checkOutABook(user.getInstitutionallId(), selectedBook.getId());
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
            String formattedString = formatter.format(transaction.getDueDate());
            String title = "Livro emprestado com sucesso.";
            String message = "[ID " + transaction.getId() + "] A devolução deve ser realizada até: " + formattedString + ".";
            txtId.setText("");
